...
/**
 * Note that this callback is NOT executed in Spigot's main thread. In case you need to perform some
 * synchronized action, please wrap your code using Spigot's scheduler.
 *
...
public interface DataAccessCallback<T> {
   ...
   * errors.
   *
   * @param entities a list of all involved entities. (Could be empty if for example no entities
   *     where found after a search.)
   */
   ...
   *
   * @param entities a list of all involved entities. (Could be empty if for example no entities
   *     where found after a search or an error occurred.)
